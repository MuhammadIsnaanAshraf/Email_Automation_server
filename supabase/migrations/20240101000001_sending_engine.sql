-- ─────────────────────────────────────────────────────────────
-- Migration: 20240101000001_sending_engine.sql
...
-- Note: We use auth.users via a view or direct reference since we don't have public.users
create or replace view public.users as
select id, email, raw_user_meta_data->>'full_name' as name
from auth.users;

-- Add daily_send_limit to a metadata table or use a separate table
...
-- Users can only see their own sends (via campaign ownership)
